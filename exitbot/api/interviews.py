...
async def list_interviews(
    ...
    if not current_user.is_hr and not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Not authorized to access interview data"
        )
...
async def update_status(
    interview_id: int,
    status: str = Query(
        ..., description="New status (in_progress, completed, archived)"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    if not current_user.is_hr and not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Not authorized to update interview status"
        )
    ...
    return {
        "status": "success",
        "message": f"Interview {interview_id} status updated to {status}",
    }
