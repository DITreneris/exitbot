...
class TestIntegration:
    ...
    def test_user_registration_and_login_flow(self, client, test_db):
        ...
        existing_user = test_db.query(User).filter(User.email == user_data["email"]).first()
    ...
    def test_employee_interview_flow(self, mock_create_client, client, employee_token, test_employee, test_db):
        ...
        message1 = {"content": "Hello, I'm ready for the interview."} # Use 'content' field
        ...
        assert response_data1["content"] == "Hello! I'll be conducting your interview today."
        
        # Verify response was saved in test_db
        db_responses1 = test_db.query(Response).filter(Response.interview_id == interview_id).all()
        assert len(db_responses1) == 1 # Should have one Response record now
        assert db_responses1[0].employee_message == message1["content"]
        assert db_responses1[0].bot_response == "Hello! I'll be conducting your interview today."
        ...
        assert response_data2["content"] == "That's interesting experience. Can you tell me about a challenge you faced?"
        
        # Verify response was saved
        db_responses2 = test_db.query(Response).filter(Response.interview_id == interview_id).order_by(Response.created_at).all()
        assert len(db_responses2) == 2
        assert db_responses2[1].employee_message == message2["content"]
        assert db_responses2[1].bot_response == "That's interesting experience. Can you tell me about a challenge you faced?"

        # Step 4: Employee sends third message
        message3 = {"content": "I once had to optimize a slow-performing database query that was causing timeout issues."}
        ...
        assert response_data3["content"] == "Thank you for sharing. Final question: what are your career goals?"

        # Verify response was saved
        db_responses3 = test_db.query(Response).filter(Response.interview_id == interview_id).order_by(Response.created_at).all()
        assert len(db_responses3) == 3
        assert db_responses3[2].employee_message == message3["content"]
        assert db_responses3[2].bot_response == "Thank you for sharing. Final question: what are your career goals?"

        # Step 5: Employee sends final message
        message4 = {"content": "I want to become a senior developer and eventually move into a tech lead role."}
        ...
        # Verify response was saved
        db_responses4 = test_db.query(Response).filter(Response.interview_id == interview_id).order_by(Response.created_at).all()
        assert len(db_responses4) == 4
        assert db_responses4[3].employee_message == message4["content"]
        assert db_responses4[3].bot_response == "Thank you for completing the interview."
        ...
        assert messages_data[1]["content"] == "Hello! I'll be conducting your interview today."
        # ... check others if needed ...
        assert messages_data[6]["role"] == "user"
        assert messages_data[6]["content"] == "I want to become a senior developer and eventually move into a tech lead role."
        assert messages_data[7]["role"] == "assistant"
        assert messages_data[7]["content"] == "Thank you for completing the interview."

    def test_hr_admin_workflow(self, client, admin_token_user, test_admin_user, test_employee, test_db):
        ...
        interview_create_data = {
            ...
            "exit_date": (datetime.utcnow() + timedelta(days=30)).strftime('%Y-%m-%d') # Example exit date
        ...
        # Verify in DB
        new_interview_db = test_db.query(Interview).filter(Interview.id == new_interview_id).first()
        ...
        our_interview = next((i for i in interviews_data["items"] if i["id"] == new_interview_id), None) # Iterate over items
        ...
        with patch("exitbot.app.api.endpoints.interviews.ReportingService.generate_interview_summary") as mock_generate_summary:
            mock_report_data = {
                "id": 1,
                "interview_id": new_interview_id,
            ...
            assert generate_report_response.status_code == 200 # Assuming sync generation for test
            ...
            retrieved_report_data = get_report_response.json()
            assert retrieved_report_data["interview_id"] == new_interview_id
    ...
    def test_full_application_flow(self, client, test_db):
        ...
        with patch("exitbot.app.llm.factory.LLMClientFactory.create_client") as mock_get_llm_client_flow:
            mock_llm_flow = MagicMock()
            mock_llm_flow.chat.return_value = "Thank you for your answer." # Mock response
        ...
        with patch("exitbot.app.api.endpoints.interviews.ReportingService.generate_interview_summary") as mock_generate_summary_flow:
            ...
            retrieved_report_data_flow = get_report_response_flow.json()
