...
def mock_ollama_generate(model, prompt, **kwargs):
    ...
    if "follow-up question" in prompt.lower():
        ...
        if "better offer" in prompt.lower() or "culture" in prompt.lower():
            return mock_ollama_response("Can you tell me more about what specifically attracted you to the new opportunity?")
        else:
...
class MockLLM:
    ...
    def patch_llm():
        ...
        return patch('exitbot.app.llm.factory.llm_client.generate_response', MockLLM.mock_generate_response)
    ...
    def patch_sentiment():
        ...
        """
        return patch('exitbot.app.llm.factory.llm_client.analyze_sentiment', MockLLM.mock_analyze_sentiment)
    ...
    def mock_generate_response(prompt, temperature=0.7):
        ...
        if "follow up" in prompt.lower():
            ...
            return {
                "response": "0.2"  # Slightly positive
            }
        else:
            return {
    ...
    def mock_analyze_sentiment(text):
        ...
        if any(word in lower_text for word in ['happy', 'great', 'excellent', 'good', 'love', 'enjoy']):
            return 0.8
        elif any(word in lower_text for word in ['okay', 'fine', 'alright', 'neutral']):
            return 0.0
        elif any(word in lower_text for word in ['unhappy', 'bad', 'poor', 'hate', 'terrible']):
            return -0.8
        else:
