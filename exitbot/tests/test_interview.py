...
def test_unauthorized_start_interview(client, test_db, employee_token, test_employee, test_hr):
...
def test_process_message(client, test_db, employee_token, test_employee):
    ...
    with patch("exitbot.app.llm.factory.LLMClientFactory.create_client") as mock_create_client:
        ...
        message_payload = {"content": "I got a better offer elsewhere."} # Matches MessageCreate schema
        response = client.post(
        ...
        db_response = test_db.query(Response).filter(
        ...
        assert db_response.bot_response == "This is a mock LLM response." # Verify mock response was saved
...
def test_complete_interview(client, test_db, employee_token, test_employee):
    ...
    update_to_progress_response = client.put(
        ...
        headers={"Authorization": f"Bearer {employee_token}"}, # Employee should be able to update own status
        json=update_to_progress_payload
    )
    assert update_to_progress_response.status_code == 200
    assert update_to_progress_response.json()["status"] == InterviewStatus.IN_PROGRESS.value
