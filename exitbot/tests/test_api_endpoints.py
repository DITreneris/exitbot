...
class TestAPIEndpoints:
    ...
    def test_token(self):
        ...
        return create_access_token(
            subject_email="test@example.com", is_admin=False
        )
    ...
    def admin_token(self):
        ...
        return create_access_token(
            subject_email="admin@example.com", is_admin=True
        )
    ...
    def test_current_user(self, client: TestClient, test_employee: User, employee_token: str):
    ...
    def test_create_interview(self, client: TestClient, test_db: Session, test_admin_user: User, admin_token_user: str, test_employee: User):
    ...
    def test_get_interview(self, client: TestClient, test_db: Session, test_admin_user: User, admin_token_user: str, test_employee: User):
        ...
        assert data["status"] == InterviewStatus.IN_PROGRESS.value # Check the status we set
    ...
    def test_list_interviews(self, client, test_db: Session, admin_token_user, test_employee, test_admin_user):
    ...
    def test_update_interview(self, client, test_db, admin_token_user, test_employee, test_admin_user):
    ...
    def test_delete_interview(self, client, test_db: Session, admin_token_user, test_employee, test_admin_user):
        ...
        interview_before = test_db.query(Interview).filter(Interview.id == interview_id).first()
        assert interview_before is not None

        # Make the delete request
        response = client.delete(
            f"/api/interviews/{interview_id}",
            headers={"Authorization": f"Bearer {admin_token_user}"} # Use the token directly
        )
        ...
        interview_after = test_db.query(Interview).filter(Interview.id == interview_id).first()
        assert interview_after is None
    
    @patch("exitbot.app.llm.factory.LLMClientFactory.create_client")
    def test_send_message(self, mock_create_client, client: TestClient, test_db: Session, test_employee: User, employee_token: str):
    ...
    def test_get_interview_messages(self, client: TestClient, test_db: Session, test_employee: User, employee_token: str):
    ...
    def test_role_based_access_control(self, client: TestClient, test_employee: User, employee_token: str, test_admin_user: User, admin_token_user: str):
        ...
        print(f"Testing employee access to {admin_endpoint} with token {employee_token[:10]}...")
        ...
        print(f"Testing admin access to {admin_endpoint} with token {admin_token_user[:10]}...")
