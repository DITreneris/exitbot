...
async def get_dashboard_statistics(
    ...
    for status_val in ["scheduled", "in_progress", "completed", "cancelled"]:
        count = (
            ...
            .filter(models.Interview.status == status_val)
            .scalar()
        )
    ...
    recent_interviews = (
        ...
        .filter(models.Interview.created_at >= start_date)
        .count()
    )
    ...
    if total_interviews > 0:
        completion_rate = (
            interviews_by_status.get("completed", 0) / total_interviews * 100
        )
    else:
        completion_rate = 0
    ...
    avg_sentiment = (
        db.query(func.avg(models.Response.sentiment))
...
async def get_activity_timeline(
    ...
    group_by: str = Query(
        "day", description="Grouping interval", pattern="^(day|week|month)$"
    ),
    ...
    activity_data = (
        ...
        .filter(models.Interview.created_at >= start_date)
        .group_by(date_trunc)  # Group by the strftime result
        .order_by(date_trunc)  # Order by the strftime result
        .all()
    )
    ...
    result = [
        dashboard_schemas.ActivityData(
            date=data.date, count=data.count  # Already formatted by strftime
        )
...
async def get_user_activity(
    ...
    total_interviews = (
        db.query(func.count(models.Interview.id))
        .filter(models.Interview.employee_id == user_id)
        .scalar()
    )
    ...
    if completed_interviews > 0:
        completed_interview_data = (
            db.query(models.Interview.created_at, models.Interview.completed_at)
            .filter(
                models.Interview.employee_id == user_id,
                models.Interview.status == "completed",
    ...
    logger.info(
        f"User activity statistics for user {user_id} retrieved by admin {current_user.id}"
    )
    return user_stats
...
async def get_interview_insights(
    ...
    if hasattr(models, "Report"):
        reports = (
            ...
            .filter(models.Report.interview_id.in_(interview_ids))
            .all()
        )
    ...
    for report in reports:
        ...
        if (
            hasattr(report, "satisfaction_score")
            and report.satisfaction_score is not None
        ):
...
async def export_dashboard_data(
    ...
    start_date: Optional[datetime] = Query(
        None, description="Start date for export (ISO format)"
    ),
    end_date: Optional[datetime] = Query(
        None, description="End date for export (ISO format)"
    ),
    ...
    for interview in interviews:
        ...
        if hasattr(models, "Message"):
            message_count = (
                db.query(func.count(models.Message.id))
                .filter(models.Message.interview_id == interview.id)
                .scalar()
            )
        ...
        interview_data.append(
            {
                ...
                if hasattr(interview, "interview_type")
                else None,
            }
        )
