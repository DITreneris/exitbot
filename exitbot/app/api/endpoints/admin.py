...
@router.delete(
    ...
    responses={
        ...
        401: {"description": "Unauthorized"},
        403: {
            "description": "Forbidden - Admin privileges required or attempt to delete self"
        },
        404: {"description": "User not found"},
    },
)
    ...
    *,
    ...
    for interview_id in update_data.interview_ids:
        try:
            ...
            if not interview:
                failures.append({"id": interview_id, "error": "Interview not found"})
                continue

            # Update interview status
            crud.interview.update(
                db, db_obj=interview, obj_in={"status": update_data.new_status}
            )
            success_count += 1

        except Exception as e:
            failures.append({"id": interview_id, "error": str(e)})
...
async def get_system_logs(
    current_user: models.User = Depends(deps.get_current_active_superuser),
    limit: int = Query(
        100, description="Maximum number of log entries to return", le=1000
    ),
