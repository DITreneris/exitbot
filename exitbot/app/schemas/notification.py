...
class NotificationCreate(BaseModel):
    ...
    reference_id: Optional[int] = Field(
        None, 
        description="Optional ID of the referenced entity (e.g., interview ID)"
    )
    reference_type: Optional[str] = Field(
        None, 
        description="Optional type of the referenced entity (e.g., 'interview')"
    )
    ...
    def validate_reference_type(cls, v, info):
        ...
        values = info.data
        if 'reference_id' in values and values.get('reference_id') is not None and v is None:
            raise ValueError("reference_type must be provided if reference_id is provided")
        return v
...
class NotificationUpdate(BaseModel):
    ...
    def validate_read_at(cls, v, info):
        ...
        values = info.data
        if 'status' in values and values.get('status') == NotificationStatus.READ and v is None:
...
class Notification(BaseModel):
    ...
    priority: NotificationPriority = Field(..., description="Priority level of the notification")
    status: NotificationStatus = Field(..., description="Status of the notification")
    recipient_id: int = Field(..., description="User ID of the recipient")
    reference_id: Optional[int] = Field(None, description="Optional ID of the referenced entity")
    reference_type: Optional[str] = Field(None, description="Optional type of the referenced entity")
...
class NotificationBulkUpdateResponse(BaseModel):
    ...
    success_count: int = Field(..., description="Number of notifications successfully updated")
