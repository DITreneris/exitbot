...
from .response import Response as ResponseSchema
...
class Message(MessageCreate):
    ...
    updated_at: Optional[datetime] = Field(
        None, description="Time the message was last updated"
    )
...
class InterviewBase(BaseModel):
    ...
    interview_type: InterviewType = Field(
        default=InterviewType.EXIT, description="Type of interview"
    )
    template_id: Optional[int] = Field(
        None, description="ID of the template used for this interview"
    )
    scheduled_at: Optional[datetime] = Field(
        None, description="Time when the interview is scheduled"
    )
    interview_metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional metadata about the interview"
    )
...
class InterviewCreate(BaseModel):
    ...
    employee_id: int = Field(
        ..., description="User ID of the employee being interviewed"
    )
    title: str = Field(..., description="Interview title")
    status: InterviewStatus = Field(
        default=InterviewStatus.SCHEDULED, description="Current status of the interview"
    )
...
class InterviewUpdateBase(BaseModel):
    ...
    status: Optional[InterviewStatus] = Field(
        None, description="Current status of the interview"
    )
    start_date: Optional[datetime] = Field(
        None, description="Time when the interview started"
    )
    scheduled_at: Optional[datetime] = Field(
        None, description="Scheduled time for the interview"
    )
    end_date: Optional[datetime] = Field(
        None, description="Time when the interview ended"
    )
    completed_at: Optional[datetime] = Field(
        None, description="Time when the interview was completed"
    )
    interview_metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata about the interview"
    )
    exit_date: Optional[date] = Field(None, description="Exit date of the interview")
    ...
    def check_dates(self):
        ...
        if (
            self.completed_at
            and self.start_date
            and self.completed_at < self.start_date
        ):
...
class InterviewInDBBase(InterviewUpdateBase):
    ...
    updated_at: Optional[datetime] = Field(
        None, description="Time the interview was last updated"
    )
    completed_at: Optional[datetime] = Field(
        None, description="Time the interview was completed"
    )
    start_date: Optional[datetime] = Field(
        None, description="Start date of the interview"
    )
...
class Interview(InterviewInDBBase):
    ...
    responses: List[ResponseSchema] = Field(
        [], description="List of responses associated with the interview"
    )
...
class InterviewWithInitialMessage(Interview):
    ...
    initial_message: Optional[Message] = Field(
        None, description="The initial message/question from the bot"
    )
...
class ReportTheme(BaseModel):
    ...
    sentiment: Optional[float] = Field(
        None, description="Sentiment score for this theme (-1.0 to 1.0)"
    )
    examples: List[str] = Field(
        default_factory=list, description="Example quotes related to this theme"
    )
...
class Report(BaseModel):
    ...
    sentiment_score: Optional[float] = Field(
        None, description="Overall sentiment score for the interview (-1.0 to 1.0)"
    )
    recommendations: Optional[List[str]] = Field(
        None, description="Recommendations based on the interview"
    )
    generated_at: datetime = Field(..., description="Time the report was generated")
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional metadata about the report"
    )
...
class InterviewTemplate(BaseModel):
    ...
    interview_type: InterviewType = Field(
        ..., description="Type of interview this template is for"
    )
    system_prompt: str = Field(..., description="System prompt for LLM", min_length=10)
    initial_message: str = Field(
        ..., description="Initial message to display to the user", min_length=1
    )
    questions: List[str] = Field(..., description="List of questions for the interview")
    created_by_id: int = Field(
        ..., description="User ID of the admin who created the template"
    )
    created_at: datetime = Field(..., description="Time the template was created")
    updated_at: Optional[datetime] = Field(
        None, description="Time the template was last updated"
    )
...
class TemplateUpdate(BaseModel):
    ...
    system_prompt: Optional[str] = Field(
        None, description="System prompt for LLM", min_length=10
    )
    initial_message: Optional[str] = Field(
        None, description="Initial message to display to the user", min_length=1
    )
    questions: Optional[List[str]] = Field(
        None, description="List of questions for the interview"
    )
