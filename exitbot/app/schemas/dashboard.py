...
class DashboardStatistics(BaseModel):
    ...
    recent_interviews: int = Field(..., description="Number of interviews in the selected time range")
...
class UserActivityStats(BaseModel):
    ...
    total_interviews: int = Field(..., description="Total number of interviews for this user")
    ...
    completion_rate: float = Field(..., description="Percentage of completed interviews")
    avg_completion_time_minutes: Optional[float] = Field(
        None, 
        description="Average time to complete an interview in minutes"
    )
    total_messages: int = Field(..., description="Total number of messages in all interviews")
...
class InsightsData(BaseModel):
    ...
    avg_satisfaction: Optional[float] = Field(
        None, 
        description="Average satisfaction score (if available)"
    )


class ExportData(BaseModel):
    """Data export container."""
    generated_at: datetime = Field(..., description="Timestamp when the export was generated")
...
class BulkInterviewStatusUpdate(BaseModel):
    ...
    interview_ids: List[int] = Field(
        ..., 
        description="List of interview IDs to update"
    )
