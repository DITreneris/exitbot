...
def update_interview_status(
    db: Session, interview_id: int, status: str
) -> Optional[Interview]:
...
def update_interview(
    db: Session, db_interview: Interview, interview_in: Dict[str, Any]
) -> Interview:
...
def update_interview_by_id(
    db: Session, interview_id: int, update_dict: Dict[str, Any]
) -> Optional[Interview]:
    """Update an interview by ID with a dictionary of values."""
    db_interview = db.query(Interview).filter(Interview.id == interview_id).first()
    if not db_interview:
        return None

    # Update fields provided in the dictionary
    for field, value in update_dict.items():
        if hasattr(db_interview, field):
            setattr(db_interview, field, value)
