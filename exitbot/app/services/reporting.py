...
from exitbot.app.services.caching import ttl_cache, invalidate_cache
...
class ReportingService:
    ...
    def get_summary_stats(
        ...
        avg_sentiment = db.query(func.avg(Response.sentiment)).join(
        ...
        top_reasons = ReportingService._extract_exit_reasons(db, start_datetime, end_datetime)
    ...
    def _extract_exit_reasons(
        ...
        responses = db.query(Response).join(
            Interview, Response.interview_id == Interview.id
        ).filter(
    ...
    def get_department_breakdown(
        ...
        departments = db.query(
            ...
            Interview, User.id == Interview.employee_id
        ).join(
            Response, Interview.id == Response.interview_id
        ).filter(
            Interview.created_at >= start_datetime,
            Interview.created_at <= end_datetime
        ).group_by(
