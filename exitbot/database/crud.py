...
def create_exit_interview(db: Session, user_id: int, interviewer_id: int = None):
    db_interview = models.ExitInterview(
        user_id=user_id,
...
def get_exit_interview(db: Session, interview_id: int):
    return db.query(models.ExitInterview).filter(models.ExitInterview.id == interview_id).first()

def get_user_exit_interviews(db: Session, user_id: int):
    return db.query(models.ExitInterview).filter(models.ExitInterview.user_id == user_id).all()

def update_exit_interview(db: Session, interview_id: int, **kwargs):
    db_interview = db.query(models.ExitInterview).filter(models.ExitInterview.id == interview_id).first()
    if not db_interview:
        return None
...
def get_questions(db: Session, category: Optional[str] = None, skip: int = 0, limit: int = 100):
...
def create_interview_response(db: Session, interview_id: int, question_id: int, response_text: str):
...
def get_interview_responses(db: Session, interview_id: int):
    return db.query(models.InterviewResponse).filter(
...
def update_interview_response(db: Session, response_id: int, response_text: str):
    db_response = db.query(models.InterviewResponse).filter(
        models.InterviewResponse.id == response_id
    ).first()
