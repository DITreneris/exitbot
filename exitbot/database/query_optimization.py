...
def optimize_department_query(db: Session):
    ...
    interview_counts = (
        select([
            User.department,
            func.count(ExitInterview.id).label("interview_count")
        ])
    ...
    sentiment_scores = (
        select([
            User.department,
            func.avg(Answer.sentiment).label("avg_sentiment")
        ])
        .select_from(
            User.__table__
            .join(ExitInterview.__table__)
            .join(Answer.__table__)
        )
        .group_by(User.department)
...
class QueryCache:
    ...
    def invalidate(prefix: Optional[str] = None) -> None:
        ...
        if prefix:
            ...
            keys_to_remove = [k for k in QueryCache._cache.keys() if k.startswith(prefix)]
