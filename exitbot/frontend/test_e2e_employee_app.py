...
def test_employee_app_login_and_first_message(page: Page):
    ...
    )  # Placeholder starts with "Type your response"
    # Updated selector: Locate button directly by text
    ...
    user_message_in_chat = page.locator(
        f'div.user-message:has-text("{user_response_text}")'
    )
    ...
    expect(user_message_in_chat).to_be_visible(
        timeout=20000
    )  # Allow time for send + LLM response
