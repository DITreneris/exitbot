...
def _handle_request_exception(e: Exception, operation: str) -> dict:
    ...
    return {
        "status": None,
        "detail": f"Could not connect to the server during {operation}. Please check the connection and API URL.",
        "raw_text": str(e),
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token:
                ...
                return None, {
                    "status": 200,
                    "detail": "Login successful but no token received.",
...
def get_interview_details(_token, interview_id):
    ...
    try:
        headers = {"Authorization": f"Bearer {_token}"}
        response = requests.get(
            f"{API_URL}/api/interviews/{interview_id}", headers=headers, timeout=10
        )
        if response.status_code == 200:
            return response.json(), None
        else:
...
def get_questions(_token):
    ...
    try:
        headers = {"Authorization": f"Bearer {_token}"}
        response = requests.get(
            f"{API_URL}/api/questions/", headers=headers, timeout=10
        )
        if response.status_code == 200:
            return response.json(), None
        else:
...
def create_question(token, question_text):
    ...
    try:
        ...
        payload = {
            "text": question_text,
            "is_active": True,
        }  # Assuming default is active
        response = requests.post(
            f"{API_URL}/api/questions/", headers=headers, json=payload, timeout=10
        )
...
def update_question(token, question_id, question_text=None, is_active=None):
    ...
    try:
        ...
        response = requests.put(
            f"{API_URL}/api/questions/{question_id}",
            headers=headers,
            json=payload,
            timeout=10,
        )
...
def delete_question(token, question_id):
    ...
    try:
        ...
        response = requests.delete(
            f"{API_URL}/api/questions/{question_id}", headers=headers, timeout=10
        )
...
def get_employee_access_details(email, full_name):
    ...
    try:
        ...
        response = requests.post(
            f"{API_URL}/api/auth/employee-access", json=payload, timeout=20
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 409:  # Conflict (e.g., name/dept mismatch)
                ...
                error_details["detail"] = error_details.get(
                    "detail",
                    "A user with this email exists, but the provided name or department does not match.",
                )
...
def update_system_config(token, config_data: dict):
    ...
    #     return None, {'status': None, 'detail': f"An unexpected error occurred during {operation}.", 'raw_text': str(e)}
    logging.info(
        f"{operation}: Placeholder function called with data: {config_data}. Clearing cache and returning input data."
    )
...
def create_interview_session(
    ...
    title: str = "Exit Interview",
):
    """
    Create a new interview session with the predefined questions format.

    Args:
        token: Auth token for the API
        employee_id: ID of the employee
        exit_date: Optional exit date in string format (YYYY-MM-DD)
        title: Title of the interview (defaults to "Exit Interview")

    Returns:
        tuple: (interview_data, error) - interview_data contains the initial question
    """
    operation = "creating interview session"
    try:
        headers = {
        ...
        payload = {"employee_id": employee_id, "title": title, "status": "scheduled"}

        if exit_date:
            payload["exit_date"] = exit_date

        response = requests.post(
            f"{API_URL}/api/interviews/start", json=payload, headers=headers, timeout=15
        )
        ...
        if response.status_code == 200:
            ...
            if responses and len(responses) > 0:
                first_question = responses[0].get("bot_response", "")
                # Add the initial question as a field for easy access
                interview_data["initial_message"] = {
                    "content": first_question,
                    "role": "assistant",
                }

            logging.info(
                f"Successfully created interview with ID: {interview_data.get('id')}"
            )
...
def send_interview_message(
    token, interview_id: int, message: str, question_id: Optional[int] = None
):
    """
    Send a message in an interview with improved handling for the predefined questions format.

    Args:
        token: Auth token for the API
        interview_id: ID of the interview
        message: User's message text
        question_id: Optional question ID (not required for predefined questions flow)

    Returns:
        tuple: (response_data, error) - response_data includes question progression data for predefined questions
    """
    ...
    try:
        ...
        # Simplified payload - predefined questions format doesn't need question_id
        ...
        if response.status_code == 200:
            ...
            logging.info(f"Message sent successfully to interview {interview_id}")
            logging.debug(f"Response data: {data}")
            return data, None
        elif response.status_code == 429:
            # Special handling for rate limiting
            logging.warning(f"Rate limit exceeded for interview {interview_id}")
            return None, {
                "status": 429,
                "detail": "The server is experiencing high load. Please try again in a moment.",
                "raw_text": response.text,
            }
        elif response.status_code == 404:
            # Special handling for missing interview
            logging.error(f"Interview {interview_id} not found")
            return None, {
                "status": 404,
                "detail": "Interview session not found. Please restart the interview.",
                "raw_text": response.text,
            }
        else:
            return None, _parse_error(response)
        ...
        logging.error(f"Connection error during {operation}")
        return None, {
            "status": None,
            "detail": "Connection error. Please check your internet connection.",
            "raw_text": "Connection Error",
        }
    except requests.exceptions.RequestException as e:
        return None, _handle_request_exception(e, operation)
    except Exception as e:
...
def get_user_profile(_token):
    """Fetch the current user's profile. Returns (profile_dict, None) or (None, error_details)."""
    operation = "fetching user profile"
    try:
        headers = {"Authorization": f"Bearer {_token}"}
        response = requests.get(
            f"{API_URL}/api/users/me", headers=headers, timeout=10
        )
        if response.status_code == 200:
            return response.json(), None
        else:
        ...
        return None, {
            "status": None,
            "detail": f"Request error during {operation}: {str(e)}",
            "raw_text": str(e),
        }
    except Exception as e:
        logging.error(f"Unexpected error during {operation}: {str(e)}")
