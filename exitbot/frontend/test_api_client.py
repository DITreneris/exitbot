...
import os
import json
import logging # Import logging to potentially check logs
...
def assert_error_details(error_details, expected_status=None, expected_detail_contains=None):
    ...
    if expected_status:
        assert error_details["status"] == expected_status, f"Expected status {expected_status}, got {error_details['status']}"
    if expected_detail_contains:
        assert expected_detail_contains in error_details["detail"], f"Expected detail '{expected_detail_contains}' not found in '{error_details['detail']}'"
...
def test_login_success(requests_mock):
    ...
    requests_mock.post(f"{MOCK_API_URL}/api/auth/login", json=mock_response, status_code=200)
...
def test_login_failure_unauthorized(requests_mock):
    ...
    requests_mock.post(f"{MOCK_API_URL}/api/auth/login", text=raw_error_text, status_code=401)

    token, error = api_client.login("test@example.com", "wrongpassword")
    assert token is None
    assert_error_details(error, expected_status=401, expected_detail_contains="Authentication failed")
...
def test_login_connection_error(requests_mock):
    ...
    requests_mock.post(f"{MOCK_API_URL}/api/auth/login", exc=requests.exceptions.ConnectTimeout)

    token, error = api_client.login("test@example.com", "password123")
    assert token is None
    assert_error_details(error, expected_status=None, expected_detail_contains="Could not connect")
...
def test_get_all_interviews_success(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/interviews?skip=0&limit=100", json=mock_data, status_code=200)
...
def test_get_all_interviews_failure_500(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/interviews?skip=0&limit=100", status_code=500, text="Server Error")

    interviews, error = api_client.get_all_interviews(mock_token)
    assert interviews is None
    assert_error_details(error, expected_status=500, expected_detail_contains="Server error")
...
def test_get_interview_details_success(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/interviews/{interview_id}", json=mock_data, status_code=200)
...
def test_get_interview_details_not_found_404(requests_mock, mock_token):
    ...
    interview_id = 999
    requests_mock.get(f"{MOCK_API_URL}/api/interviews/{interview_id}", status_code=404, text="Not Found")

    details, error = api_client.get_interview_details(mock_token, interview_id)
    assert details is None
    assert_error_details(error, expected_status=404, expected_detail_contains="resource not found")
...
def test_get_interview_summary_success(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/reports/interview/{interview_id}/summary", text=mock_summary, status_code=200)
...
def test_get_interview_summary_not_available_404(requests_mock, mock_token):
    ...
    interview_id = 8
    requests_mock.get(f"{MOCK_API_URL}/api/reports/interview/{interview_id}/summary", status_code=404, text="Not Found")
...
def test_get_interview_summary_failure_500(requests_mock, mock_token):
    ...
    interview_id = 9
    requests_mock.get(f"{MOCK_API_URL}/api/reports/interview/{interview_id}/summary", status_code=500, text="Processing Error")
...
def test_get_summary_stats_success(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/reports/summary", json=mock_data, status_code=200)
...
def test_get_summary_stats_failure_403(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/reports/summary", status_code=403, text="Forbidden")

    stats, error = api_client.get_summary_stats(mock_token)
    assert stats is None
    assert_error_details(error, expected_status=403, expected_detail_contains="Authorization denied")
...
def test_get_department_breakdown_success(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/reports/by-department", json=mock_data, status_code=200)
...
def test_export_data_success(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/reports/export?format=json", content=mock_content, status_code=200)
...
def test_get_questions_success(requests_mock, mock_token):
    ...
    matcher = requests_mock.get(f"{MOCK_API_URL}/api/questions/", json=mock_data, status_code=200)
...
def test_get_questions_failure_500(requests_mock, mock_token):
    ...
    requests_mock.get(f"{MOCK_API_URL}/api/questions/", status_code=500, text="DB Error")
...
def test_create_question_success(requests_mock, mock_token):
    ...
    post_matcher = requests_mock.post(f"{MOCK_API_URL}/api/questions/", json=mock_response, status_code=201)
    # Mock the GET request that will happen after cache clear
    get_matcher = requests_mock.get(f"{MOCK_API_URL}/api/questions/", status_code=200, json=[mock_response])

    result, error = api_client.create_question(mock_token, question_text)
    assert result == mock_response
    assert error is None
    assert post_matcher.last_request.json() == {"text": question_text, "is_active": True}
    ...
    assert get_matcher.call_count >= 1 # Needs to be called at least once after the POST
...
def test_create_question_failure_400(requests_mock, mock_token):
    ...
    requests_mock.post(f"{MOCK_API_URL}/api/questions/", status_code=400, text=raw_error_text)

    result, error = api_client.create_question(mock_token, question_text)
    assert result is None
    assert_error_details(error, expected_status=400, expected_detail_contains="Text cannot be empty") # Check parsed detail
    assert error["raw_text"] == raw_error_text
...
def test_update_question_success(requests_mock, mock_token):
    ...
    put_matcher = requests_mock.put(f"{MOCK_API_URL}/api/questions/{question_id}", json=mock_response, status_code=200)
    get_matcher = requests_mock.get(f"{MOCK_API_URL}/api/questions/", status_code=200, json=[mock_response])

    result, error = api_client.update_question(mock_token, question_id, question_text=updated_text)
    assert result == mock_response
    assert error is None
...
def test_update_question_not_found_404(requests_mock, mock_token):
    ...
    question_id = 99
    requests_mock.put(f"{MOCK_API_URL}/api/questions/{question_id}", status_code=404, text="Not Found")
...
def test_delete_question_success(requests_mock, mock_token):
    ...
    question_id = 2
    delete_matcher = requests_mock.delete(f"{MOCK_API_URL}/api/questions/{question_id}", status_code=204)
    get_matcher = requests_mock.get(f"{MOCK_API_URL}/api/questions/", status_code=200, json=[])
...
def test_delete_question_not_found_404(requests_mock, mock_token):
    ...
    question_id = 99
    requests_mock.delete(f"{MOCK_API_URL}/api/questions/{question_id}", status_code=404, text="Not Found")
...
def test_delete_question_failure_500(requests_mock, mock_token):
    ...
    question_id = 1
    requests_mock.delete(f"{MOCK_API_URL}/api/questions/{question_id}", status_code=500, text="Deletion Failed")
...
def test_get_employee_access_details_success(requests_mock):
    ...
    mock_data = {"access_token": "emp_token", "employee_id": 123, "token_type": "bearer"}
    payload = {"email": "e@c.co", "full_name": "Emp Name", "department": "Dept"}
    requests_mock.post(f"{MOCK_API_URL}/api/auth/employee-access", json=mock_data, status_code=200)
...
def test_get_employee_access_details_failure_409(requests_mock):
    ...
    mock_error_json = {"detail": "Provided name does not match existing record for this email."}
    requests_mock.post(f"{MOCK_API_URL}/api/auth/employee-access", json=mock_error_json, status_code=409)

    result, error = api_client.get_employee_access_details(**payload)
    assert result is None
    # Check specific detail parsed from JSON
    assert_error_details(error, expected_status=409, expected_detail_contains="Provided name does not match")
