...
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".")) # Assumes script is in project root
sys.path.insert(0, project_root)

# --- Imports from your application ---
# Adjust these paths if your structure is different
try:
    from exitbot.app.db.database import db_connection # Import the connection manager instance
    from exitbot.app.schemas.user import UserCreate
    from exitbot.app.db.crud.user import create_user, get_user_by_email
    from exitbot.app.core.security import get_password_hash # Import for optional password update
except ImportError as e:
    print(f"Error importing application modules: {e}")
    print("Please ensure the script is run from the project root directory and the virtual environment is active.")
    sys.exit(1)


# --- Admin User Details ---
ADMIN_EMAIL = "admin@example.com"
ADMIN_PASSWORD = "password" # The password you want to use
...
def create_admin_user():
    ...
    try:
        ...
        if user:
            ...
            print(f"Admin user '{db_user.email}' created successfully with ID: {db_user.id}.")
